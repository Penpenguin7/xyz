...
def config_from_flags():
  ...
  """
  if (FLAGS.melody_encoder_decoder, FLAGS.melody_config).count(None) != 1:
  ...
  if FLAGS.melody_encoder_decoder is not None:
    ...
    if FLAGS.melody_generator_id is not None:
      generator_details = magenta.protobuf.generator_pb2.GeneratorDetails(
          id=FLAGS.melody_generator_id)
      if FLAGS.melody_generator_description is not None:
        generator_details.description = FLAGS.melody_generator_description
    else:
    ...
    if FLAGS.melody_config not in melody_rnn_model.default_configs:
      raise MelodyRnnConfigFlagsException(
          '`--config` must be one of %s. Got %s.' % (
              melody_rnn_model.default_configs.keys(), FLAGS.melody_config))
    config = melody_rnn_model.default_configs[FLAGS.melody_config]
    config.hparams.parse(FLAGS.melody_hparams)
    if FLAGS.melody_generator_id is not None:
      config.details.id = FLAGS.melody_generator_id
    if FLAGS.melody_generator_description is not None:
      config.details.description = FLAGS.melody_generator_description
    return config
